...
// var를 사용해서 선언한 변수는 프로그램 어디에서 선언하던 항상 맨 위로 끌어올려진다.
...
// var
// varibale hoisting => 아래에 선언된 var 선언을 맨 위로 끌어올린다.
// 어디에 선언했냐에 상관없이 항상 제일 위로 변수 선언을 끌어올린다.
// var는 block scope를 사용하지 않고 fuction scope를 사용한다. => 유연성
// block scope => 변수가 선언된 {} 블록 내부에서만 사용할 수 있다. => let, const
// fuction scope => 변수가 선언된 함수의 {} 블록 내부에서만 사용할 수 있다. => var
// block sope는 변수가 선언된 {} 블록 밖으로 나가면 소멸되고 function scope는 변수가
// 선언된 함수가 종료되면 소멸된다.
console .log('age = ' + age);
var age = 100;
console .log('age = ' + age);
...
let globalName = '장길산';
{
	...
	// let name = '임꺽정'; // let은 같은 이름의 변수를 선언할 수 없다.
	name = '임꺽정';
	console.log('name = ' + name);
